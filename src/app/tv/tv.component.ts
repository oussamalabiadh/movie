...
export class TvComponent implements OnInit, OnDestroy {
    ...
    setTimeout(() => {
      this.moviesSubscrip = this._MoviesService
        ...
        .subscribe({
          ...
          complete: () => {
            this._MoviesService.shardPageNumberTv.next(this.activatePageNum()); // send activate number to shard movie service
            if (this.firstRun) {
              this.firstRun = false;
              this.isLoding = false;
            }
  ...
  pervPage(): void {
    if (this.activatePageNum() > 1) {
      ...
      this._Router.navigate([`/tv/${(curentActivate -= 1)}`]).finally(() => {
        this.getMovies(this.activatePageNum());
      });
    }
  }
